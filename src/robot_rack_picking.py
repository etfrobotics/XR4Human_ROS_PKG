...
import datetime
import requests
from BrainWatchPanda.msg import RobotAction 
...
def GoForBIN(P_appraoch, P_C, dxa=0, dya=0, dza=0):
    ...
    pandica.move_robot_L_absolute(P_C[0:3],P_C[3:],dx=dxa,dy=dya,dz=dza)
    pandica.move_robot_L_absolute(P_C[0:3],P_C[3:])
    pandica.grasp_client(0.00,0.005,0.005,0.25,10)
    pandica.move_robot_L_absolute(P_C[0:3],P_C[3:],dx=0,dy=0,dz=dza)
    pandica.move_robot_L_absolute(P_C[0:3],P_C[3:],dx=0,dy=dya,dz=dza)
    pandica.set_joints(P_appraoch[0],P_appraoch[1],P_appraoch[2],P_appraoch[3],P_appraoch[4],P_appraoch[5],P_appraoch[6])
...
def action_callback(data):
    ...
    return_rack_id = data.return_rack_id
    pick_rack_id = data.pickup_rack_id
    
    if (return_rack_id == 1):
        PutBackBin(P1_approach,P1_1C,0,-0.05,0.05)
    if (return_rack_id == 2):
    ...
    #     #Pick bin 1_1
    #     GoForBIN(P2_approach,P2_1C,0,-0.1,0.1)
    ...
    #     PutBackBin(P2_approach,P2_1C,0,-0.1,0.1)
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        rospy.Subscriber('robot_action',RobotAction,action_callback)
        rospy.spin()
